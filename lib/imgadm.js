...
function ActivateZcage(pool) {
  ...
  const zcageds = [pool + ZCAGE.BASEDIR, pool + ZCAGE.VMS, pool + ZCAGE.IMAGES];
  const basedir = pool + ZCAGE.BASEDIR;
  const vmdir = pool + ZCAGE.VMS;
  const imgdir = pool + ZCAGE.IMAGES;
...
function docker_list(library) {
  const r = spawnSync('list-tags.sh', [library]);
  if (r.error) {
    console.log(r.error);
    return -1;
  }
...
function docker_pull(library, tag) {
  const r = spawnSync('pfexec', ['pull-by-tag.sh', library, tag]);
...
function container_images(callback) {
  const obj = [];
  const cmd = 'ls /zcage/images | wc -l ';
  ...
  if (fs.existsSync(ZCAGE.INDEX)) {
    fs.stat(ZCAGE.INDEX, function(err, stats) {
      ...
      const seconds = (new Date().getTime() - new Date(stats.mtime).getTime()) /
                1000;
        ...
        fs.readdir(ZCAGE.IMAGES, function(err, item) {
          item.forEach(function(item, index, arr) {
            fs.stat(ZCAGE.IMAGES + '/' + item, function(err, stats) {
              ...
              const img = {};
              const r = spawnSync('sha256sum ' + ZCAGE.IMAGES + '/' + item, {
                shell: true,
              });
              const errorText = r.stderr.toString().trim();
              if (errorText) {
              ...
              const id = r.stdout.toString().split(/\s+/);
              img.Id = id[0];
...
function image_id2fname(id, callback) {
  ...
  const images = JSON.parse(data);
  const imgid = images.filter(function(e) {
    return e.Id == id;
  });
  if (imgid.length == 0) {
    if (callback) {
...
function create_image(query, callback) {
  ...
  if (!query.from_source && !query.from_image && query.repo && query.tag) {
    ...
    const img = image_id2fname(query.from_image);
    if (img == null) {
      if (callback) {
        callback(404, {
          message: 'No such image',
    ...
    const ext = img.filename.split('.').slice(1).join('.');
    const name = query.repo + '-' + query.tag + '.' + ext;
