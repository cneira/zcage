...
const OMNIOS_URL = 'https://downloads.omnios.org/media/img.json';
const OMNIOS_IMAGES = 'https://downloads.omnios.org/media/';
const ARCH = 'amd64';
...
const ZCAGE = {
    ...
    OMNIOS_IMAGES: '/zcage/images/omnios_index.json',
...
function filter_omnios_images(err, resp, data) {
    if (err) {
        console.log("error retrieving data");
        return null;
    }
    ...
    var cols = columnify(ds);
    console.log(cols);
    fs.writeFileSync(ZCAGE.OMNIOS_IMAGES, JSON.stringify(vms, 4, null));
}
...
function GetUUIDdata(err, resp, data) {
    ...
    if (err) {
        ...
        d = fs.readFileSync(ZCAGE.OMNIOS_IMAGES, "utf-8");
        omnios_vms = JSON.parse(d);
        if (omnios_vms.error)
            console.log(vms.error);
    ...
    fs.readdir(ZCAGE.IMAGES, function(err, items) {
        ...
        for (var i = 0; i < items.length; i++) {
            if (items[i].split('.').slice(1).join('.') == 'zss.gz') {
                ...
                omnios_objs.push(omnios_vms.images.filter(function(e) {
                    return e.uuid == items[i].split('.')[0].toString();
                })[0]);
        ...
        if (objs.length > 0 && objs[0] != undefined) {
            console.log("\nLocally available Joyent images");
            console.log("------------------------------------");
            objs.forEach(function(e) {
	        ds.push ( { UUID: e.uuid, NAME: e.name, VERSION: e.version, OS: e.os, PUBLISHED: e.published_at});	
            });
	    console.log(columnify(ds));
        }
        if (omnios_objs.length > 0 && omnios_objs[0] != undefined ) {
	   	
            console.log("\nLocally available Omnios images");
            console.log("------------------------------------");
	    ds = []	;
            omnios_objs.forEach(function(e) {
	        ds.push ( { UUID: e.uuid, NAME: e.name, VERSION: e.version, OS: e.brand});	
            });
	    console.log(columnify(ds));
        }
...
function list_omnios_images() {
    let opts = {
        url: OMNIOS_URL
    };
    fetchmeta(opts, filter_omnios_images);
}

function get_omnios_rpath(uuid, rpath) {

    if (fs.existsSync(ZCAGE.OMNIOS_IMAGES)) {
        data = fs.readFileSync(ZCAGE.OMNIOS_IMAGES, "utf-8");
        let vms = {};
        vms = JSON.parse(data);
        if (vms.error)
            console.log(vms.error);
        ...
        var ds = [];
        for (var i = 0; i < vms.images.length; i++) {
            var obj = vms.images[i];
            if (obj.uuid == uuid) {
                path = obj.path
                return path;
            }
        ...
        console.log("Need to run images --list avail --provider omnios");
        return null;
    }
}
...
function getzss(uuid, source) {
    ...
    switch (source) {
        ...
        case 'omnios':
            let rpath;
            rpath = get_omnios_rpath(uuid, rpath);
            fileUrl = OMNIOS_IMAGES + rpath;
            PATH = ZCAGE.IMAGES + '/' + uuid + '.zss.gz';
            break;
...
function can_create_vm() {
    testfile = ZCAGE.VMS.toString() + "/.test"
    try {
        fs.writeFileSync(testfile, "just testing access");
...
module.exports.list_omnios_images = list_omnios_images;
