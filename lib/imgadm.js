...
function getzss(uuid, source) {
    ...
    if (fs.existsSync(PATH)) {
        ...
        var canWrite = true;
        try {
            fs.writeFileSync(ZCAGE.IMAGES + '/.test','permission check');
            fs.unlinkSync(ZCAGE.IMAGES + '/.test', 'permission check');
        } catch (e) {
            canWrite = false;
        }
        if (canWrite) {
...
function docker_list(library) {
    var r = spawnSync('tools/list-tags.sh', [library]);
    if (r.error) {
        console.log(r.error);
        return -1;
    }
...
function docker_pull(library, tag) {
    var r = spawnSync('pfexec', ['tools/pull-by-tag.sh', library, tag]);
