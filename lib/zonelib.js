...
function start(zonename) {
    ...
    if (vnicused(zone.zonename)) {
        console.log("Error: VNIC is being used by other container");
        return null;
    }
...
function validate_zonespec(z) {
    var schema = fs.readFileSync('/zcage/conf/zoneschema.json', 'utf8');
    var ajv = new Ajv({
        allErrors: true
    });
    if (!validateips(z.net)) {
	console.log("network parameters are not correct or missing", z.net);
	return null;
   }
