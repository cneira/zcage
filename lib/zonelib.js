...
function install(zonename, brand, uuid) {
    ...
    if (brand == 'lx') {
        ...
        iz = spawnSync('pfexec', ['zoneadm', '-z', zonename, 'install', '-s', img], {
            shell: true
        });
...
function create_zone_spec(resources) {
    ...
    if (spec.brand === "bhyve") {
        var bhyve_metadata = {
            ...
            type: 'generic',
            vnc: "unix=/tmp/" + uuidv4() + ".vnc,wait",
            bootrom: "BHYVE_DEBUG",
            console: "socket,/tmp/" + uuidv4() + ".com1",
            vcpus: "cpus=1,sockets=1,cores=1,threads=2"
        };
        spec.bhyve_metadata = bhyve_metadata;
        Object.keys(resources.bhyve_metadata).forEach(function (key) {

            if (typeof (resources.bhyve_metadata[key]) != undefined) {
                console.log("key =", key);
    ...
    if (debug.enabled) {
        ...
        console.log("vhyve", JSON.stringify(spec.bhyve_metadata, null, 4));
    }
...
function spec2script(spec) {
                ...
                Object.keys(spec.rctl).forEach(function (key) {
                    if (key == "max-physical-memory" ||
                        ...
                        script += "set name=zone.cpu-shares;" +
                            "add value (priv=privileged, limit=" +
                            spec.rctl["cpu-shares"] + ",action=none);end;";
                    }
                });
                break;
    ...
    if (debug.enabled)
        console.log(arguments.callee.name, script);
        ...
        parser.parseString(data, function (err, result) {
            ...
            Object.keys(tmp).forEach(function (key) {
                ...
                if (!(key in oldrctl)) {
                    ...
                    if (pid) {
                        rctl = spawnSync('pfexec', ['prctl', '-n', `zone.${key}`, '-s', '-v', tmp[key], pid], {
                            shell: true
                        });
                        if (rctl.err)
                ...
                if (key == "zone.max-physical-memory" ||
                    ...
                    if (tmp[key] != undefined) {
                        if (key == "dedicated-cpu") {
                            ...
                            script += ";end;";
                            if (pid) {
                                rctl = spawnSync('pfexec', ['prctl', '-n', `${key}`, '-r', '-v', tmp[key], pid], {
                                    shell: true
                                });
                                if (rctl.err)
                                    console.log("Error updating resources ", rctl.stderr.toString());
                            ...
                            if (pid) {
                                rctl = spawnSync('pfexec', ['prctl', '-n', `zone.${key}`, '-r', '-v', tmp[key], pid], {
                                    shell: true
                                });
                                if (rctl.err)
                                    console.log("Error updating resources ", rctl.stderr.toString());
