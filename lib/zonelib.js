...
function rctl(zname, rctlOptions) {
    ...
    update_rctl(zname, rctlobj["rctl"], Asynczonecfgexec);
}
...
function update_rctl(zname, newrctl, callback) {
    ...
    fs.readFile('/etc/zones/' + zname + '.xml', function(err, data) {
	   ...
	   //TODO: make it generic
	    ...
	    if ( 'cpu-shares' in newrctl) 
            ...
            var zinitpid = spawnSync('pgrep', ['-z', zname, 'init']);
           ...
           Object.keys(tmp).forEach(function(key) {
	        if(!(key in oldrctl)) {
		   console.log("adding new rctl ", key, tmp[key]);
                    script += "add rctl ; ";
                    script += " set name=" + key + ";" 
