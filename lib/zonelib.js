...
function listzones(listOptions, out) {
	...
	if (!out) {
		console.log("UUID\t\t\t\t\t TYPE\tSTATE\tRAM(MB)\t ALIAS\t\t\t CREATED");
		for (i = 0, len = zoneobjs.length; i < len; i++) {
			if (zoneobjs[i].zonename != "global") {
			getzonedata(zoneobjs[i].zonename, function (zdata, z) {
				let ram;
			if (zdata.brand == 'bhyve' || zdata.brand == 'kvm') {
				var obj = zdata.vm_data;
				if (obj.ram.includes('G'))
					ram = bytes.parse(obj.ram).toString() * 1024 ;
				else 
					ram = bytes.parse(obj.ram).toString()  ;
			} else {
				var obj = zdata.memory;
				ram = obj['zone.max-physical-memory'] / 1024 / 1024;
			}
				let created ;
				if(zdata.metadata)
				...
				if (z.state == "running") {
					console.log(chalk `${z.uuid}\t ${type}\t{green.bold ${z.state}}\t${ram}\t${z.zonename}\t${created.CreatedAt}`);

				} else {
					console.log(chalk `${z.uuid}\t ${type}\t{red.bold ${z.state}}\t${ram}\t${z.zonename}\t${created.CreatedAt}`);
				}
			}, zoneobjs[i]);
		  }
		}
