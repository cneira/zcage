...
function install(zonename, brand) {

    if (brand == 'lx') {
        console.log("installing brand lx");

	var iz = spawnSync('pfexec', ['zoneadm', '-z', zonename, 'install', '-s', '/tmp/centos68.zss.gz'], {shell: true});
...
function build(createOptions) {
    ...
    if ("net" in createOptions) {
        ...
        delete createOptions["with-image"];
