...
function create(zonename, zone_spec) {
    ...
    if (script != null) {
        ...
        if (quota in zone_spec) {
            var cmd = "set quota=" + zone_spec.quota + " " + zone_spec.zonepath;
            var quota = spawnSync('pfexec', ['zfs', cmd]);
            if (quota.err != null) {
                console.log("Setting quota", quota.stderr.toString());
                return null;
            }
        }
    }
        ...
        switch (key) {
            ...
            case "rctl":
                ...
                Object.keys(spec.rctl).forEach(function(key) {
                    if (key == 'quota') {
                        var cmd = "set quota=" + spec.rctl.quota + " " + spec.zonepath;
                        var quota = spawnSync('pfexec', ['zfs', cmd]);
                    } else
