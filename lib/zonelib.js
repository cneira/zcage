...
function build(createOptions) {
    ...
    if ("net" in createOptions) {
        ...
        if ( ("ram" in  createOptions) || ("max-lwps" in createOptions)) {
        ...
        var schema = fs.readFileSync('./lib/schemas/zoneschema.json', 'utf8'); 
	var ajv = new Ajv(); 
	...
	if (!valid) console.log(validate.errors);
        ...
        console.log(chalk `{red.bold Error} missing --net=nic|ipaddr|gateway`);
        return null;
    }
}
...
function update_rctl(zname, newrctl, callback) {
    ...
    fs.readFile('/etc/zones/' + zonename + '.xml', function(err, data) {
	   ...
	   //TODO: make it generic
	    ...
	    if ( 'dedicated-cpu-importance' in newrctl) 
            tmp["dedicated-cpu-importance"] = newrctl['dedicated-cpu-importance'];
	   ...
	   Object.keys(result.zone.pset).forEach(function(key) {
		...
		let min = obj['ncpu_min'];
           ...
           Object.keys(tmp).forEach(function(key) {
		    ...
		    if (tmp["dedicated-cpu-importance"] != undefined) {
			script += "set importance=" + tmp["dedicated-cpu-importance"]+ ";";
			}   
		   script+= ";end";
		   } else { 
                    script += "add rctl ; ";
