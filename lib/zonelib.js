...
function create(zonename, zone_spec) {
    ...
    if (script != null) {
        ...
        if (zone_spec.brand == 'lx') {
            status = install(zonename, zone_spec.brand, zone_spec['with-image'], zone_spec);
            if (status != 0) {
...
function spec2script(spec) {
    ...
    Object.keys(spec).forEach(function(key) {
        switch (key) {
            ...
            default:
                if (key != 'with-image' && key != 'alias' && key != 'debug' && key != 'docker')
...
function build(createOptions) {
    ...
    if ("net" in createOptions) {
        ...
        if (!createOptions.docker && createOptions['with-image'] && !fs.existsSync(zfs.ZCAGE.IMAGES + '/' + createOptions['with-image'] + '.zss.gz')) {
            console.log("--with-image: image has not been pulled, first execute: pfexec zcage pull --image <uuid>");
            return null;
        }
        ...
        if (createOptions.brand == 'lx' && !("with-image" in createOptions) && !createOptions.docker) {
            console.log("--with-image option is required for a lx container");
            return null;
        }
        if (createOptions.brand != 'lx' && ("with-image" in createOptions)) {
            console.log(
                "--with-image option is only valid for a lx brand zone"
            );
            return null;
        }
