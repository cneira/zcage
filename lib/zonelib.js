...
function listzones(listOptions, out) {
	...
	if (!out) {
		console.log("UUID\t\t\t\t\t TYPE\tSTATE\tRAM(MB)\t ALIAS");
		for (i = 0, len = zoneobjs.length; i < len; i++) {
			...
			if (zdata.brand == 'bhyve' || zdata.brand == 'kvm') {
				...
				if (z.state == "running") {
					console.log(chalk `${z.uuid}\t ${type}\t{green.bold ${z.state}}\t${ram}\t${z.zonename}`);

				} else {
					console.log(chalk `${z.uuid}\t ${type}\t{red.bold ${z.state}}\t${ram}\t${z.zonename}`);
				}
			}, zoneobjs[i]);
		  }
		}
