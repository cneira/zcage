...
function create(zonename, zone_spec) {
    ...
    if (script != null) {
        ...
        if ("quota" in zone_spec.rctl) {
            ...
            var ds = "rpool" + zone_spec['zonepath'];
        ...
        switch (key) {
            ...
            case "rctl":
                ...
                Object.keys(spec.rctl).forEach(function(key) {
                    if (key == 'quota') {
                        script += "add attr; set name=quota;set type=string;" +
                            " set value=" + spec.rctl.quota + ";end;";
                    } else
                    ...
                    switch (key) {
                        case 'vnc':
                            script +=
                                ...
                                "\"" +
                                " ;end;";
                            script += "add attr; set type=string;" +
                                "set name=vnc-port;" +
                                "set value=" +
    ...
    if ("net" in createOptions) {
        ...
        if (createOptions.brand === "bhyve") {
            createOptions.bhyve_metadata = {
                ...
                disk: createOptions.bootdisk,
                bootdisk: createOptions.bootdisk,
...
function isvnc_enabled(zone) {
    getzonedata(zone.zonename, function(zdata) {
        ...
        var unixds = zone['zone-path'] + '/root' +
            ...
            " " + obj['vnc-port'];
        var tmp = zone['zone-path'] + '/root/tmp/vnc.log';
        out = fs.openSync(tmp, 'a'),
            err = fs.openSync(tmp, 'a');
        console.log("unixds", unixds);
        var vnc = spawn("pfexec /usr/lib/brand/bhyve/socat " + unixds, {
            shell: true,
            detached: true,
