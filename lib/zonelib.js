...
function rctl(zname, rctlOptions) {
    console.log("zname: " + zname + " options : " + JSON.stringify(rctlOptions, null, 4));
    if (!isAbletoexec()) {
	console.log(
	    "You must be root or use an account with Primary Administrator Role to Activate execute this action"
	);
		...
		if (!(key in oldrctl)) {
			...
			if (key == "autoboot") {
				...
				if(oldrctl["zone.cpu-shares"]){
				    script += " select rctl name=zone.cpu-shares;"
				    script +=
					"remove value (priv=privileged,limit=" +
					oldrctl["zone.cpu-shares"] + ",action=none);";
				    script += "end;";
				}
				script +=
				    "add dedicated-cpu ; ";
...
function container_update(zdef, callback) {
