...
const zones = ZonesString().map( z => Zonedata(z))
const nonGlobalZones = zones.filter(zone => zone.zonename != "global");
...
/**
 * Returns an array of zone objects, We ignore the global zone, and zones in
 * the configured state. You can filter the results by providing `listOptions`
 * 
 * @example
 * // returns All zones, except the global
 * listzones();
 * @example
 * // returns a list of running zones
 * listzones({state: "running"});
 * 
 * @param {Object} listOptions - Options to filter by
 * @return {Array[Object]} - An array of objects containing
 *		id,name,state,uuid,brand, 'ip-type' for each zone
 * */
function listzones(listOptions=[]) {
	const unconfigured = (zone) =>  zone.state != "configured";
	let zoneobjs = [];
	let zones = nonGlobalZones.filter(unconfigured)
		.map(z => { if(z.state == "installed") z.state = "stopped"; return z });

	if ("state" in listOptions)
		zones = zones.filter(zone => zone.state == listOptions.state);
	
	zones.map((z) => { zoneobjs.push(z) });

	return zoneobjs;
}

function displayZones(zoneobjs) {
	let data = [];
	console.log(
	...
	for (i = 0, len = zoneobjs.length; i < len; i++) {
		getzonedata(zoneobjs[i].zonename, function (zdata, z) {
			...
			let type = brandName(zdata.brand);
...
module.exports.displayZones = displayZones;
