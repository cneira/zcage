	...
	Object.keys(spec).forEach(function (key) {
		...
		default:
			if (key != 'with-image' && key != 'alias' && key != 'debug' && key !=
				'docker' && key != 'udata' && key != 'cpu' && key != 'image')
...
function create_container(zdef, callback) {
	...
	zdef = convert_zjson_to_coptions(zdef);
	var r = validate_brand_reqs(zdef);
	if (r.statusCode != '201') {
		callback(400, r);
		return;
	}
	var z = create_zone_spec(zdef);
...
function convert_zjson_to_coptions(zdef) {
	...
	zdef.zonepath = `/zcage/vms/${zdef.alias}`;
...
function container_logs(zname, callback) {
	var script = "";
	var r;
	if (validator.isUUID(zname)) {
		z = getdata(zname, zname);
...
function container_export(zname, callback) {
	...
	script = 'pfexec zfs send ' + ds + ' | pfexec gzip >  /tmp/' + uuid +
		'.zss.gz';
	r = spawnSync(script, {
		shell: true
	});
	...
	script = 'pfexec mv  /tmp/' + uuid + '.zss.gz /zcage/images/';
	r = spawnSync(script, {
		shell: true
	});
	...
	if (callback) {
		callback(200, {
			message: "Exported image:" + z.uuid + " to /images/" + uuid + ".zss.gz"
		});
	}
	return;
}
