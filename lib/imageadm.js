...
function ActivateZcage() {
    ...
    var pool = GetPool();
    var zcageds = [pool + '/zcage', pool + '/zcage/vms', pool + '/zcage/images', pool + 'zcage/conf'];
    ...
    fs.writeFile("/zcage/conf/zoneschema.json", JSON.stringify(JSON.parse(schema), null, 4), function(err) {
        if (err) {
