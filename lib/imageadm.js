...
function writefile(err, resp, body) {
    if (err) throw err;
    var output = paths.ZCAGEIMAGES + '/' + resp.req.path.split('/')[2] + '.zss.gz';
...
function Isactivated() {
    if (fs.existsSync(paths.ZCAGEDS) && fs.existsSync(paths.LXBRAND) && fs.existsSync(paths.BHYVEBRAND)) {
        return 0;
    } else {
        console.log("zcage needs to be activated or lx and bhyve brands are missing");
        return null;
    }
}
