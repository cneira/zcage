...
function fetch(url, callback) {
	...
	fs.writeFile(output, body, function (err) {
	    console.log("VM image downloaded succesfully");
	if(callback) {
	  callback(201, {message: "Image created successfully"});
		return;
	}
	});
    }
	...
	fs.stat(ZCAGE.INDEX, function (err, stats) {
		...
		fs.readdir(ZCAGE.IMAGES, function (err, item) {
			...
			fs.stat(ZCAGE.IMAGES + '/' + item, function (err, stats) {
				...
				let vm = 'hypervisor';
				img.RepoTags = ['local', vm + ':bhyve|kvm' ];
				img.Labels = {type: ['KVM', 'BHYVE']};
			    } else if (!item.includes('native')) {
				img.Labels = {type: ['LX'], provider: 'Joyent'};
				img.RepoTags = ['local', 'lx:joyent' ];
			    } else {
				img.Labels = {type: ['OS'], provider: 'OmniOSCE'};
				img.RepoTags = ['local', 'SunOS:5.11' ];
			    }
			    obj.push(img);
			    if (obj.length >= cnt) {
...
function create_image(query, callback) {
    var dockerimg='';
    if(query.repo && query.tag) {
	dockerimg = docker_pull(query.repo, query.tag) 
	var name = "docker-" + query.repo + "-" + query.tag;
	let cmd = "pfexec mv " + dockerimg + " " + ZCAGE.IMAGES + "/" + name + "-" + uuidv4() + ".gz";
	console.log("creating docker image: " + cmd);
	var r = spawnSync(cmd, {
	    shell: true
	});
	if (r.error){
	    if(callback) {
		callback(500, {message: r.message});
	    }
	} else {
	    if(callback) {
		callback(200, {message: "Container image " + name + " created" });
		r = spawnSync("pfexec rm " + dockerimg , {
		    shell: true
		});
		if (r.error){
		    if(callback) {
    ...
    } else if (query.from_source) {
	fetch(query.from_source, callback);		
    }	
}
