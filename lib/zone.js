...
function halt(zonename, callback) {
	...
	callback(500, {message: errorText});
...
function getzonedata(zname, callback, zobj) {
	...
	parser.parseString(data, function (err, result) {
		...
		if (zone.ram.toUpperCase().includes('G'))
		    zone.ram = bytes.parse(zone.ram).toString() * 1024 ;
		else
		    zone.ram = bytes.parse(zone.ram).toString() ;
...
function create_container(zdef, callback) {
    ...
    if (zconverted == null) {
	callback(500, {message: "Non valid option for brand"});
	return;
    }
...
function validate_brand_reqs(zdef) {
	...
	if (!zdef['with-image'] && !zdef.docker) {
	    ...
	    resp= { statusCode: 400, message: "docker tag is incorrect should be repo/tag or library/official-distro tag"};
	}else if (zdef.docker) {
...
function container_logs(zname, stdout, stderr, callback) {
    ...
    if (z.state != "running"){
	if(callback){
	    callback(400, {message: "Container is not running"});
	    return;
	}
...
function network_connect(netid, body, callback) {
	...
	fs.readFile(zfs.ZCAGE.NETWORKS, (err, data) => {
	    ...
	    if (netw.length == 0) {
		if(callback)
		    callback(404,{message: "No such network."});
		return;
	    }
	    netw = netw[0];
	...
	callback(500, {message: "No networks available"});
	return;
    }
}
...
function network_disconnect(netid, body, callback) {
	...
	callback(500, {message: "Not able to disconnect network"});
	return;
    }
}
...
function network_create_vnic(body, callback) {
	...
	if(!arr[index + 1]){
	    if(callback)
		callback(200, {message: "vnic " + net.name +
...
function network_create(net, callback) {
	...
	fs.readFile(zfs.ZCAGE.NETWORKS, (err, data) => {
	    ...
	    if(dups.length >0) {
		callback(500, {message: "Duplicated network name"});
		return;
	    }
	...
	var nobj = { id: uuidv4(), Name: net.Name, Config: net.Config };
	objs.push(nobj);
