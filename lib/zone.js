...
function ZonesString() {
    ...
    if (!zoneadm.stdout) {
        console.log("Account using zcage must be root or elevate privileges by sudo or pfexec");
        return process.exit();
    }
...
function setupzone(zonename, zone_spec) {
    ...
    switch (zone_spec.brand) {
        ...
        case "sparse":
        case "pkgsrc":
            status = start(zonename, null, null, true);
            if (status != 0) {
...
function validate_zonespec(z) {
    var schema = {
        ...
        "properties": {
            ...
            "brand": {
                "type": "string",
                "enum": ["sparse", "dpkg", "ipkg", "lipkg", "lx", "bhyve", "kvm", "pkgsrc"]
            },
            "ip-type": {
                "type": "string",
                "enum": ["exclusive"]
