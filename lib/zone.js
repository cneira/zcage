...
function install(zonename, brand, uuid, zone_spec) {
    ...
    if ((brand == 'bhyve' || brand == 'kvm') && zone_spec['with-image']) {
        ...
        iz = spawnSync('pfexec', ['tools/bhyve-zvol.sh', img, zone_spec.rctl.quota,
        ...
        if (zone_spec['uses-cloud-init'] == true) {
            if (!zone_spec.udata) {
                ...
                iz = spawnSync('pfexec', ['tools/cloud-init.sh', JSON.stringify(zone_spec, null,
                    4)]);
    ...
    switch (zdef.brand) {
        case 'lx':
            if (!zdef['with-image'] && !zdef.docker) {
                ...
                console.log("docker is " + zdef.docker);
                if (zdef.docker.indexOf("library") == -1)
                	var repo = zdef.docker.split('/');
                else
	                var repo = zdef.docker.split(' ');
                var r = spawnSync('tools/list-tags.sh', [repo[0]]);
                if (r.error) {
                    console.log(r.error);
                    resp = {
                        statusCode: 400,
    ...
    if (fs.existsSync(zfs.ZCAGE.NETWORKS)) {
        fs.readFile(zfs.ZCAGE.NETWORKS, (err, data) => {
            ...
            netw.Config.forEach(function(e, index, arr) {
                ...
                var zonecfg = spawnSync('pfexec', ['zonecfg', '-z', z.zonename, script]);
                var errorText = zonecfg.stderr.toString().trim();
                if (errorText) {
                    console.log("error on zonecfg executing script", zonecfg.stderr.toString());
                    if (callback)
...
function run_container_config(zonename, attr) {
    ...
    if (entry) {
        ...
        var child = spawn("pfexec zlogin " + zonename + " /usr/bin/env " + entry, {
            shell: true,
            detached: true,
            stdio: 'ignore'
        });
