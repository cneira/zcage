...
function run_container_config(zonename, attr) {
    ...
    zonedata = zfs.ZCAGE.IMAGES + "/" + zonename + ".json";
    if (fs.existsSync(zonedata)) {
        d = fs.readFileSync(zonedata, "utf-8");
        conf = JSON.parse(d);
        if (conf.error)
 ...
 */
function container_run_entrypoint(zname, args) {
    var zoption = "-z";
    var z = "";
    if (validator.isUUID(zname)) {
        z = getdata(zname, zname);
    ...
    if (z == null) {
        console.log(chalk `[{red.bold ERR}]  ${zname} does not exists   `);
        return;
    }

    zonedata = zfs.ZCAGE.IMAGES + "/" + zname + ".json";
    if (fs.existsSync(zonedata)) {
	...
	if(debug.enabled)
	    console.log("args " +  zname + " /usr/bin/env " + entry + " " + cmd + " " + args);

        if (z.state == "running") {
            var zlogin = spawn("zlogin " + zname + " /usr/bin/env " + entry + " " + cmd + " " + args, {
                shell: true,
                detached: true,
                stdio: "ignore",
            });
