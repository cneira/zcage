...
function build(createOptions) {
    ...
    if ("net" in createOptions) {
        ...
        //if (vnicused(zname)) {
        //   console.log("vnic in used by other container");
        //  return null;
        // }
...
function container_run_entrypoint(zname, cmd) {
    ...
    if (z.state == 'running') {
        ...
        var zlogin = spawnSync('pfexec', ['/usr/sbin/zlogin', zname, entry, cmd], {
            shell: true
        });
        var errorText = zlogin.stderr.toString().trim();
        if (errorText)
        ...
        var start = spawnSync('pfexec', ['zoneadm', zoption, zname, 'boot'], {
            shell: true
        });
        var errorText = start.stderr.toString().trim();
        zone_status = 'off';
