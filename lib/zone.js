...
function ZonesString() {
  const zoneadm = spawnSync('zoneadm', ['list', '-cp']);
  ...
  const zones = zoneadm.stdout.toString().split('\n');
  zones.pop();
  return zones;
}

function Zonedata(zonestring) {
  const values = zonestring.toString().split(':');
  const keys = ['zoneid', 'zonename', 'state', 'zone-path', 'uuid', 'brand',
    'ip-type',
  ];
  const m = new Map();
  values.pop();
...
function getdata(zonename, uuid) {
  const zones = list();
  const zone = zones.filter((zone) => (zone.zonename == zonename || zone.uuid ==
        uuid));
...
function destroy(zonename, dzvol, callback) {
  ...
  if (z.state == 'running') {
    const zhalt = spawnSync('pfexec', ['zoneadm', zoption, zonename,
      'halt',
    ]);
  ...
  if (dzvol == true && (z.brand == 'bhyve' || z.brand == 'kvm')) {
    getzonedata(z.zonename, function(zdata) {
      const obj = zdata.vm_data;
      const zvol = obj.bootdisk;
      if (zvol) {
        const zvol_destroy = spawnSync('pfexec', ['zfs',
          'destroy', zvol,
        ]);
        const errorText = zvol_destroy.stderr.toString().trim();
        if (errorText) {
          console.log(errorText);
...
function getinfo(zonename) {
  const zonecfg = spawnSync('zonecfg', ['-z', zonename, 'info']);
  const errorText = zonecfg.stderr.toString().trim();
  if (errorText) {
...
function exec(zonename, cmd, out) {
  const zlogin = spawnSync('pfexec', ['/usr/sbin/zlogin', zonename, cmd]);
  const errorText = zlogin.stderr.toString().trim();
  if (errorText) {
...
function fixup_dataset(zonename, args, out) {
  const fixup = spawnSync('pfexec', args, {
    shell: true,
  });
  const errorText = fixup.stderr.toString().trim();
  if (errorText) {
...
function start(zonename, opts, callback, quiet) {
  ...
  if ((zone.brand == 'bhyve' || zone.brand == 'kvm') && (opts != undefined &&
            opts != null)) {
    const script = 'add fs; set dir=' + '"' +
            opts + '"' +
            ';set special=' + '"' +
            opts + '"' +
            '; set type=lofs;add options ro;' +
            'add options nodevices;end;';
    const r = spawnSync('pfexec', ['zonecfg', zoption, zonename,
  ...
  const start = spawnSync('pfexec', ['zoneadm', zoption, zonename, 'boot']);
  const errorText = start.stderr.toString().trim();
  ...
  getzonedata(zonename, function(zdata) {
    if (zdata.Cmd) {
      const logs = zdata.Cmd.replace(',', ' ') + ' 2> ' +
                '/stderr.log' + ' 1> ' + '/stdout.log';
      const Cmd = ['zlogin', zdata.name, logs];
      const execmd = spawnSync('pfexec', Cmd);
      if (debug.enabled) {
...
function Asynczonecfgexec(err, zname, script, callback) {
  ...
  const zonecfg = spawnSync('pfexec', ['zonecfg', '-z', zname, script]);
  if (zonecfg.err) {
...
function halt(zonename, callback, quiet) {
  ...
  const stop = spawnSync('pfexec', ['zoneadm', zoption, zonename, 'halt']);
  const errorText = stop.stderr.toString().trim();
...
function reboot(zonename, callback) {
  ...
  if (z.state == 'running') {
    if (z.brand == 'bhyve' || z.brand == 'kvm') {
      const path = z['zone-path'] + '/root/tmp';
      let vncpid = spawnSync('pgrep', ['-f', path]);
      const pid = vncpid.stdout.toString();
    ...
    const errorText = reboot.stderr.toString().trim();
    if (errorText) {
      console.log('info ', errorText);
      if (callback) {
        callback(500, {
...
function uninstall(zonename) {
  const uz = spawnSync('pfexec', ['zoneadm', '-z', zonename, 'uninstall']);
  if (uz.error != null) {
...
function create(zonename, zone_spec) {
  const script = spec2script(zone_spec);
  ...
  let status = -1;
  if (script != null) {
    const zonecfg = spawnSync('pfexec', ['zonecfg', '-z', zonename, script]);
    if (debug.enabled) {
    ...
    if ('quota' in zone_spec.rctl) {
      const cmd = 'quota=' + zone_spec.quota + ' ';
      const ds = zfs.GetPool() + zone_spec.zonepath;

      if (debug.enabled) {
        console.log('Setting quota=%s on dataset %s', zone_spec.rctl.quota, ds);
      }

      const quota = spawnSync('pfexec', ['zfs', 'set', 'quota=' +
  ...
  Object.keys(spec).forEach(function(key) {
    switch (key) {
      case 'net':
        spec.net.forEach(function(e) {
          ...
          if (e.address == 'dhcp') {
            ...
            script += 'set physical=' + e.physical + ';';
            let prefix = ip.IPv4.parse(e.netmask.toString())
                .prefixLengthFromSubnetMask();
            const addr = e.address + '/' + prefix +
                            ';';
      ...
      case 'resolvers':
        ...
        for (let i = 0, len = spec.resolvers.length; i < len; i++) {
          if (i + 1 >= len) {
...
function build(createOptions) {
  ...
  if ('net' in createOptions || 'docker' in createOptions) {
    ...
    const z = create_zone_spec(createOptions);
    ...
    if (create(zname, z) === 0) {
      ...
      if ('docker' in createOptions) {
        const tags = createOptions.docker;
...
function addrctl(createOptions, zname) {
  ...
  if (createOptions.ram && (z.brand == 'bhyve' || z.brand == 'kvm')) {
    ...
    if (createOptions.ram.includes('mb')) {
...
function update_rctl(zname, newrctl, callback, callback2) {
  ...
  if (z.brand == 'bhyve' || z.brand == 'kvm') {
    let script;
    if ('quota' in newrctl) {
      const cmd = 'quota=' + newrctl.quota + ' ';
      const ds = zfs.GetPool() + z['zone-path'];
      const quota = spawnSync('pfexec', ['zfs', 'set',
    ...
    const r = spawnSync('pfexec', ['zonecfg', '-z', zonename, script]);
    if (r.error != null) {
    ...
    parser.parseString(data, function(err, result) {
      ...
      if (result.zone.rctl) {
        Object.keys(result.zone.rctl).forEach(function(key) {
          const obj = result.zone.rctl[key];
          const value = obj['rctl-value'][0];
      ...
      if (result.zone.pset) {
        Object.keys(result.zone.pset).forEach(function(
            key) {
          const obj = result.zone.pset[key].$;
          const max = obj.ncpu_max;
          const min = obj.ncpu_min;
      ...
      const zinitpid = spawnSync('pgrep', ['-z', zonename,
        'init',
      ]);
    ...
    parser.parseString(data, function(err, result) {
      ...
      if (result.zone.attr) {
        Object.keys(result.zone.attr).forEach(function(
            key) {
          const obj = result.zone.attr[key];
      ...
      if (result.zone.network) {
        Object.keys(result.zone.network).forEach(function(
            key) {
          const obj = result.zone.network[key];
          if (obj['net-attr']) {
            Object.keys(obj['net-attr']).forEach(function(key) {
              const net = obj['net-attr'][key].$;
              let ip; let gw;
              if (net.name == 'ips') {
...
function vnicused(zonename, array) {
  ...
  const nic_wanted = [];
  const parser = new xml2js.Parser();
  const running = listzones(obj, true);
  let result = null;
  let data;
...
function validate_zonespec(z) {
  const schema = {
    ...
    'properties': {
      ...
      'brand': {
        'type': 'string',
        'enum': ['sparse', 'dpkg', 'ipkg', 'lipkg', 'lx', 'bhyve', 'kvm', 'pkgsrc', 'illumos'],
      },
      'ip-type': {
        'type': 'string',
        'enum': ['exclusive'],
  ...
  const valid = validate(z);

  if (!valid) {
    console.log(chalk `{red.bold ERROR : Some Options are not valid when provisioning a zone}`);
    validate.errors.forEach(function(e) {
      console.log('* ' + e.dataPath.replace('.', '') + ': ' + e.message + ' [' + e.params.allowedValues + ']');
    });
    if (debug.enabled) {
...
function isAbletoexec() {
  const uid = spawnSync('whoami');
  const user = uid.stdout.toString().replace(/\n$/, '');
  if (user == 'root') return true;
  const prim = spawnSync('grep', ['-i', user, '/etc/user_attr']);
...
function isvnc_enabled(zone, brand) {
  getzonedata(zone.zonename, function(zdata) {
    ...
    const child = spawn('pfexec /usr/lib/brand/' + brand + '/socat ' + unixds, {
      shell: true,
      detached: true,
      stdio: 'ignore',
    });
...
function vnic_exists(zone) {
  const cmd = 'pfexec dladm show-vnic | awk \'{ if (NR != 1) {print $1}}\'';
  let rc = false;
  let nics = [];
  r = spawnSync(cmd, {
    shell: true,
...
function getattr(zname, name) {
  ...
  const script = 'info attr name=' + name;
  const r = spawnSync('pfexec',
      ['zonecfg', zoption, z.zonename, script]);
...
function get_containers(cb) {
  ...
  for (let i = 0, len = lzones.length; i < len; i++) {
...
function create_container(zdef, callback) {
  ...
  const r = validate_brand_reqs(zconverted);
  ...
  const z = create_zone_spec(zconverted);
  ...
  switch (zdef.brand) {
    case 'lx':
      if (!zdef['with-image'] && !zdef.docker) {
        ...
        const repo = zdef.docker.split('/');
        const r = spawnSync('list-tags.sh', [repo[0]]);
        if (r.error) {
          console.log(r.error);
          resp = {
            statusCode: 400,
        ...
        const jtags = JSON.parse(r.stdout.toString());
        const tags = jtags.tags;
        console.log(tags);
        let found = false;
...
function getstats(zname, callback) {
  ...
  getzonedata(z.zonename, function(z, cb) {
    const kstats = spawnSync('pfexec', ['kstat', '-j']);
    if (kstats.error) {
      if (cb) {
        cb(400, {
    ...
    const zjs = kjs.filter((e) => (e.name == z.name));
    const net = [];
    let cpu = [];
    let cpuinfo = [];
    const mstats = [];
...
function container_top(zname, flags, callback) {
  ...
  const errorText = r.stderr.toString().trim();
  if (errorText) {
    if (callback) {
      callback(500, {
        message: 'Something went wrong.',
  ...
  const procs = r.stdout.toString().split('\n');
  const psout = [];

  procs.forEach(function(e) {
    const ps = e.trim().split(/\s+/);
    psout.push(ps);
  });
...
function container_export(zname, callback) {
  ...
  const uuid = uuidv4();
  const ds = zfs.GetPool() + z['zone-path'] + '@' + uuid;
  r = spawnSync('pfexec',
      ['zfs', 'snapshot', ds]);
  ...
  while (res != '1') {
    ...
    const snaps = r.stdout.toString().split('\n');
  ...
  script = 'pfexec mv  /tmp/' + z.brand + '-' + 'omniosce' + '-' +
        uuid + '.zss.gz ' + zfs.ZCAGE.IMAGES + '/';
  r = spawnSync(script, {
    shell: true,
  });
  var errorText = r.stderr.toString().trim();
  ...
  console.log('Exported image:' + z.uuid + ' to /images/' + z.brand + '-' + 'omniosce' + '-' + uuid + '.zss.gz');
  return;
}
...
function network_connect(netid, body, callback) {
  const newphys = uuidv4();
  const zoneid = body.Container;
  const net = body.net;
  let script = '';
  if (validator.isUUID(zoneid)) {
    z = getdata(zoneid, zoneid);
  } else {
    z = getdata(zoneid);
  }
  if (z == null) {
  ...
  if (fs.existsSync(zfs.ZCAGE.NETWORKS)) {
    fs.readFile(zfs.ZCAGE.NETWORKS, (err, data) => {
      ...
      netw.Config.forEach(function(e, index, arr) {
        const newphys = 'vnic' + uuidv4().split('-')[0].slice(0, 7);
        console.log(e);
        const cmd = 'pfexec dladm create-vnic ' + newphys + ' -l ' + e.physical;
        console.log(cmd);
        r = spawnSync(cmd, {
          shell: true,
        });
        ...
        if (net.address == 'dhcp') {
          ...
          script += 'set physical=' + newphys + ';';
          const prefix = e.IPRange.split('/')[1];
          const addr = net.address + '/' + prefix +
                        ';';
          script += 'set allowed-address=' + addr +
                        ';';
          script += 'set defrouter=' + e.Gateway +
        ...
        const zonecfg = spawnSync('pfexec', ['zonecfg', '-z', z.zonename, script]);
...
function network_create_vnic(body, callback) {
  body.vnic.forEach(function(net, index, arr) {
    const cmd = 'pfexec dladm create-vnic ' + net.name + ' -l ' + net.over;
    r = spawnSync(cmd, {
      shell: true,
    });
    const errorText = r.stderr.toString().trim();
    ...
    if (!arr[index + 1]) {
      if (callback) {
        callback(200, {
          message: 'vnic ' + net.name +
                        ' over ' + net.over + ' created',
        });
      }
    }
  });
...
function network_vnic_prune(callback) {
  vnic_list(function(code, data) {
    ...
    data.networks.forEach(function(e, index, arr) {
      if (e.Usedby == '') {
        const cmd = 'pfexec dladm delete-vnic ' + e.Name;
        r = spawnSync(cmd, {
          shell: true,
        });
        const errorText = r.stderr.toString().trim();
...
function vnic_list(callback) {
  const cmd = 'pfexec dladm show-vnic | awk \'{ if (NR != 1) {print }}\'';
  const rc = false;
  r = spawnSync(cmd, {
    shell: true,
  });
  const networks = [];
  const obj = r.stdout.toString().split('\n');
  obj.pop();
  obj.forEach(function(e, index, arr) {
    ...
    if (!arr[index + 1]) {
      if (callback) {
        ...
        console.log(columnify(networks, {
          Align: 'right',
          config: {
            Name: {
              minWidth: 6,
...
function network_create(net, callback) {
  ...
  if (fs.existsSync(zfs.ZCAGE.NETWORKS)) {
    fs.readFile(zfs.ZCAGE.NETWORKS, (err, data) => {
      ...
      const networks = JSON.parse(data);
      const dups = networks.filter((e) => (e.Name == net.Name));
      if (dups.length > 0) {
        callback(500, {
      ...
      networks.forEach(function(e, index, arr) {
        ...
        if (!arr[index + 1]) {
          ...
          objs.push(nobj);
          fs.writeFile(zfs.ZCAGE.NETWORKS,
    ...
    objs.push(nobj);
    fs.writeFile(zfs.ZCAGE.NETWORKS, JSON.stringify(objs, null, 4),
        function(err) {
          if (err) {
            callback(500, {
              message: 'Something wrong happened',
...
function run_container_config(zonename, attr) {
  entry = getattr(zonename, attr);
  if (entry) {
    entry = entry.substring(2, entry.length - 2);
    console.log('executing entry point=' + entry);
    const child = spawn('pfexec zlogin ' + zonename + ' /usr/bin/env ' + entry, {
      shell: true,
      detached: true,
      stdio: 'ignore',
    });
...
function container_run_entrypoint(zname, cmd) {
  ...
  if (z.state == 'running') {
    entry = getattr(zname, 'container-config');
    entry = '';
    entry = entry.substring(2, entry.length - 2);
    if (entry == undefined) entry = '';
    if (cmd == undefined) cmd = '';
    var zlogin = spawnSync('pfexec', ['/usr/sbin/zlogin', z.zonename, entry, cmd], {
      shell: true,
    });
    ...
    console.log(zlogin.stdout.toString());
  } else {
    const start = spawnSync('pfexec', ['zoneadm', zoption, z.zonename, 'boot'], {
      shell: true,
    });
