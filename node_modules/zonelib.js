...
function rctl(zname, rctlOptions) {
    ...
    if (zone.state == "running") {
        ...
        Object.keys(rctlobj.rctl).forEach(function(key) {
            var rctl = spawnSync('pfexec', ['prctl', '-n', `zone.${key}`, '-r', '-v', rctlobj.rctl[key],pid], {shell: true});
            if (rctl.status != 0)
                console.log("Error updating resource limits on live zone\n",
                    rctl.stderr.toString()); });
    } else {
        rctlobj["rctl"] = addrctl(rctlOptions);
        var delscript = delrctlscript(zname); 
         return null;
        var zonecfg = spawnSync('pfexec', ['zonecfg', '-z', zname, delscript],{shell: true});
        if (zonecfg.status != 0)
            console.log("Error updating resource limits on stopped zone", rctl.stderr.toString());
        var script = spec2script(zname, rctlobj);
        var zonecfg = spawnSync('pfexec', ['zonecfg', '-z', zname, script],{shell: true});
        if (zonecfg.status != 0)
            console.log("Error updating resource limits on stopped zone", rctl.stderr.toString());
    }
    return start.status;
...
function create(zonename, zone_spec) {
    ...
    if (script != null) {
        ...
        console.log("Error creating", zonecfg.stdout.toString());
        console.log("Error creating", zonecfg.stderr.toString());
...
function delrctlscript(zname, callback) {
