...
function listzones(listOptions) {
    ...
    console.log("UUID\t\t\t\t\tSTATE\t\t NAME\t\t UPTIME");
    for (var i = 0, len = zoneobjs.length; i < len; i++) {
        if (zoneobjs[i].zonename != "global")
		if ( zoneobjs[i].state == "running") {	
		    addmeta(zoneobjs[i]);
	            console.log(chalk` ${zoneobjs[i].uuid}   {green.bold ${zoneobjs[i].state}}\t\t${zoneobjs[i].zonename} ${zoneobjs[i].uptime}`);
		    
		} else 
	            console.log(chalk` ${zoneobjs[i].uuid}   {red.bold ${zoneobjs[i].state}}\t\t${zoneobjs[i].zonename}`);
    }
}
    ...
    if (zoneobj.state == "running") {
        ...
        if (zoneobj.brand == "lx") {
		 ...
		 exec(zname, "uptime", out);
	         zoneobj["uptime"] = out["stdout"].split("  ")[2];
	     }
    }
}
...
function exec(zonename, cmd, out) {
   ...
   if( out != undefined)
    out["stdout"] = zlogin.stdout.toString();
